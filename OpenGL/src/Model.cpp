...
namespace {
glm::fdualquat MakeDualQuat(const glm::fquat &rotation,
...
void Model::BoneTransform(const float &i_timeInSeconds,
                          std::vector<glm::mat4> &io_transforms,
  ...
  unsigned int numPosKeys =
      m_scene->mAnimations[0]->mChannels[0]->mNumPositionKeys;
  ...
  float AnimationTime = fmod(TimeInTicks, m_scene->mAnimations[0]
                                              ->mChannels[0]
                                              ->mPositionKeys[numPosKeys - 1]
                                              .mTime);
...
void Model::loadModel(const std::string &i_path) {
  ...
  const aiScene *scene =
      importer.ReadFile(i_path, aiProcess_Triangulate | aiProcess_FlipUVs |
...
void Model::loadMeshBones(aiMesh *i_aiMesh,
  ...
  for (unsigned int i = 0; i < i_aiMesh->mNumBones; i++) {
    ...
    if (Bone_Mapping.find(BoneName) != Bone_Mapping.end()) {
      ...
      glm::fquat offsetRot =
          glm::normalize(glm::quat_cast(m_BoneInfo[BoneIndex].offset));
      glm::vec3 offsetTrans(m_BoneInfo[BoneIndex].offset[3][0],
                            m_BoneInfo[BoneIndex].offset[3][1],
                            m_BoneInfo[BoneIndex].offset[3][2]);
      m_BoneInfo[BoneIndex].offsetDQ =
          glm::normalize(MakeDualQuat(offsetRot, offsetTrans));
    }
  ...
  // Normalize bone weights per vertex to ensure proper blending (especially for
  // DQS).
...
void Model::loadAnimations(const std::string &BoneName,
                           AnimationMap &o_animations) {
...
void Model::ReadNodeHeirarchy(float AnimationTime, const aiNode *pNode,
  ...
  if (pNodeAnim) {
    ...
    TranslationM = glm::translate(
        TranslationM, glm::vec3(Translation.x, Translation.y, Translation.z));
  ...
  glm::vec3 nodeTranslation(NodeTransformation[3][0], NodeTransformation[3][1],
                            NodeTransformation[3][2]);
  glm::fdualquat NodeDQ =
      glm::normalize(MakeDualQuat(nodeRotation, nodeTranslation));
  ...
  if (Bone_Mapping.find(NodeName) != Bone_Mapping.end()) {
    ...
    m_BoneInfo[NodeIndex].FinalTransformation =
        GlobalTransformation * m_BoneInfo[NodeIndex].offset;

    glm::fdualquat finalDQ =
        glm::normalize(GlobalDQ * m_BoneInfo[NodeIndex].offsetDQ);
...
unsigned int Model::FindScaling(float AnimationTime,
                                const aiNodeAnim *pNodeAnim) {
...
unsigned int Model::FindRotation(float AnimationTime,
                                 const aiNodeAnim *pNodeAnim) {
...
unsigned int Model::FindPosition(float AnimationTime,
                                 const aiNodeAnim *pNodeAnim) {
