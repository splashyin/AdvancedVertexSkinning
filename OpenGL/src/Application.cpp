...
int main(void) {
  ...
  // Shader modelShader("res/shaders/vertex.shader",
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    for (unsigned int i = 0; i < Transforms.size(); ++i) {
      ...
      glUniformMatrix4fv(boneTransform, 1, GL_FALSE,
    ...
    // lamp_cube = glm::rotate(lamp_cube, (float)glfwGetTime(),
    // glm::vec3(0.0, 1.0, 0.0));
    lamp_cube = glm::translate(lamp_cube, lamp.getPosition());
    lamp_cube = glm::scale(
        lamp_cube,
        glm::vec3(0.2f)); // it's a bit too big for our scene, so scale it down
...

    {
      ...
      ImGui::SliderFloat("Ratio on DQS", &f, 0.0f,
